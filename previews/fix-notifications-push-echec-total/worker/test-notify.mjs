...
// par le service push (reason) doit remonter, un seul exemple par code.
{
  const s = summarizePushResults([
  ...
  if (s.reasons?.["400"] !== "BadJwtToken")
    fail("reason manquante ou incorrecte pour le code 400")
  if ("403" in (s.reasons || {}))
    fail("un outcome sans reason ne devrait pas apparaître dans reasons")
}
